...
type WriterDraft = { title: string; body: string; images: string[] };
...
export default function WriterComposer({ userId, initial, editing = false, onClose, onSave }: WriterComposerProps) {
  ...
  const handleFiles = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImages((current) => [...current, ...accepted.map((file) => URL.createObjectURL(file))]);
  ...
  const removeImage = (index: number) => {
    const remoteCount = initial?.images?.length || 0;
    setImages((current) => current.filter((_, imageIndex) => imageIndex !== index));
    if (index >= remoteCount) setPendingFiles((current) => current.filter((_, fileIndex) => fileIndex !== index - remoteCount));
  };
  ...
  const save = async (event: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const uploaded = await Promise.all(pendingFiles.slice(0, 8).map((file) => uploadStorageFile(file, `posts/${userId}/${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, '-')}`, getSessionToken())));
      const remoteImages = images.filter((image) => !image.startsWith('blob:'));
      await onSave({ title: title.trim(), body: body.trim(), images: [...remoteImages, ...uploaded].slice(0, 8) });
