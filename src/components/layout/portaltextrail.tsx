        ...
        <nav className="portal-text-rail-nav">
          {links.map(([href, label, english]) => {
            const active = href === '/' ? pathname === '/' : pathname.startsWith(href);
             if (href === '/assistant') return <button key={href} type="button" onClick={(event) => { const phase = handleNavigationClick(event, href, pathname); if (phase !== 'blank') window.dispatchEvent(new CustomEvent('gyopo-assistant-open')); }} className="portal-text-link portal-text-link-button">{language === 'ko' ? label : english}</button>;
