...
import { Maximize2, MessageCircle, Minimize2, PanelRightClose, PanelRightOpen, Send, Users } from 'lucide-react';
      ...
      <aside id="global-lounge" className={`global-lounge fixed bottom-4 right-4 top-24 z-[240] hidden w-[22rem] flex-col overflow-hidden rounded-3xl border border-cyan-200/15 bg-[#0a1120]/46 shadow-[0_24px_90px_rgba(0,0,0,.35)] backdrop-blur-xl transition-transform duration-300 lg:flex ${desktopOpen ? 'translate-x-0' : 'translate-x-[calc(100%+1rem)]'} ${desktopMaximized ? 'global-lounge-fullscreen' : ''}`}>
         <div className="border-b border-white/8 bg-[#0d1628]/45 p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <button type="button" onClick={toggleDesktopMaximized} aria-label={desktopMaximized ? '라운지 축소' : '라운지 전체 화면'} className="global-lounge-expand"><span className="sr-only">{desktopMaximized ? '축소' : '전체 화면'}</span>{desktopMaximized ? <Minimize2 size={15} /> : <Maximize2 size={15} />}</button>
...
export default function GlobalChat() {
  ...
  return (
    <>
      <aside id="global-lounge" className={`fixed bottom-4 right-4 top-24 z-40 hidden w-[22rem] flex-col overflow-hidden rounded-3xl border border-white/10 bg-[#0a1120]/78 shadow-[0_24px_90px_rgba(0,0,0,.45)] backdrop-blur-2xl transition-transform duration-300 lg:flex ${desktopOpen ? 'translate-x-0' : 'translate-x-[calc(100%+1rem)]'}`}>
        <div className="border-b border-white/8 bg-[#0d1628]/70 p-4">
         <div className="flex items-center justify-between gap-3">
       ...
       <button onClick={() => setDesktopLounge(!desktopOpen)} aria-label={desktopOpen ? '글로벌 라운지 최소화' : '글로벌 라운지 최대화'} title={desktopOpen ? '라운지 최소화' : '라운지 최대화'} className={`fixed bottom-5 z-50 hidden h-9 w-9 place-items-center rounded-xl border border-white/10 bg-[#10182b]/80 text-white shadow-2xl backdrop-blur-xl transition-all lg:grid ${desktopOpen ? 'right-[23rem]' : 'right-5'}`}>
         {desktopOpen ? <PanelRightClose size={15} className="text-teal-300" /> : <PanelRightOpen size={15} className="text-teal-300" />}
       </button>
      <div className="fixed bottom-3 left-3 right-3 z-40 lg:hidden">
      {mobileOpen && <div className="mb-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-[#10182b]">
        ...
        <button onClick={() => setMobileOpen((open) => !open)} aria-label={mobileOpen ? '실시간 라운지 최소화' : '실시간 라운지 최대화'} className="ml-auto grid h-10 w-10 place-items-center rounded-xl border border-white/10 bg-[#10182b]/88 text-white shadow-xl backdrop-blur"><MessageCircle size={17} className="text-teal-300" /></button>
    </div>
    </>
  );
}
