...
export default function AppRuntime({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    setSaveError('');
  }, [savedCountry, savedGender, user?.id]);
  ...
  const saveOnboarding = async (event: React.FormEvent) => {
    ...
    if (!nextGender || !nextCountry) {
      setSaveError('성별과 국가를 모두 선택해주세요.');
      return;
    }
    setSaving(true);
    setSaveError('');
    try {
      const completedUser = await completeProfileOnboarding(user, nextGender, nextCountry, getSessionToken());
      ...
      <dialog ref={dialogRef} onCancel={(event) => event.preventDefault()} className="m-auto w-[calc(100%-2rem)] max-w-lg overflow-hidden rounded-[2rem] border border-white/10 bg-[#10182b] p-0 text-white shadow-2xl backdrop:bg-[#050812]/90">
        {!sessionChecked ? (
          ...
          <form onSubmit={saveOnboarding} className="p-6 md:p-8">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-300">회원 활동을 시작하려면 성별과 거주 국가를 선택해주세요. 저장 후에는 일반 계정에서 변경할 수 없습니다.</p>
            ...
            <button disabled={saving || !(savedGender || gender) || !(savedCountry || country)} className="mt-6 w-full rounded-xl bg-cyan-300 py-3.5 font-black text-slate-950 transition hover:bg-cyan-200 disabled:cursor-not-allowed disabled:opacity-40">
