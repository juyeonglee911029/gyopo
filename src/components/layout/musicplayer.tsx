...
import { Heart, Music2, Pause, Play, Repeat2, Search, SkipBack, SkipForward, Volume2, VolumeX } from 'lucide-react';
...
export default function MusicPlayer({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    const closeFavorites = (event: PointerEvent) => {
      if (!favoriteMenuRef.current?.contains(event.target as Node)) setFavoriteMenuOpen(false);
    };
    document.addEventListener('pointerdown', closeFavorites);
    return () => document.removeEventListener('pointerdown', closeFavorites);
  }, []);
  ...
  const changeVolume = (next: number) => {
    ...
    window.localStorage.setItem('gyopo-music-volume', String(bounded));
  ...
  return (
    <section ref={searchShellRef} className={`music-player-shell ${embedded ? 'music-player-embedded' : ''}`} aria-label="음악 플레이어">
      <div className="music-player-bar">
        ...
        <div className="music-player-search">
          ...
          <input aria-label="음악 검색" value={query} onFocus={() => setSearchFocused(true)} onBlur={() => window.setTimeout(() => setSearchFocused(false), 160)} onChange={(event) => setQuery(event.target.value)} className="min-w-0 w-full bg-transparent text-sm text-white outline-none placeholder:text-slate-500" />
        </div>
        <div className="music-player-options">
           ...
           <div ref={favoriteMenuRef}>
            ...
            {favoriteMenuOpen && <div className="music-favorites-popover border-0 bg-black/80 p-2 text-white shadow-2xl backdrop-blur-2xl">
          ...
          <iframe ref={frameRef} onLoad={() => { subscribeToPlayerState(frameRef.current); syncFrame(); }} title="GYOPO music player" src={`https://www.youtube.com/embed/${MUSIC_TRACKS[0].videoId}?enablejsapi=1&origin=${encodeURIComponent(SITE_URL)}&autoplay=1&cc_load_policy=0&iv_load_policy=3&playsinline=1`} className="pointer-events-none absolute h-px w-px opacity-0" allow="autoplay; encrypted-media" />
      </div>

      {searchFocused && <div className="music-player-results">
        ...
        {query && <div className="mt-2 grid gap-2 sm:grid-cols-2">
          {results.length ? results.map((item) => <div key={item.id} className={`flex min-w-0 items-center gap-2 border-0 px-2 py-2 ${item.id === track.id ? 'bg-teal-300/10' : 'bg-white/[.05]'}`}>
            <button type="button" onClick={() => selectTrack(item)} className="flex min-w-0 flex-1 items-center gap-2 border-0 text-left outline-none ring-0">
              ...
              <span className="min-w-0">
                ...
                <span className="block truncate text-[10px] text-slate-500">{item.views || 'YouTube 검색 결과'}{item.published ? ` · ${item.published}` : ''}</span>
              </span>
            </button>
            <button type="button" onClick={() => toggleFavorite(item)} aria-label="즐겨찾기" className={`shrink-0 border-0 outline-none ring-0 ${favoriteIds.includes(item.id) ? 'text-rose-300' : 'text-slate-500'}`}><Heart size={14} fill={favoriteIds.includes(item.id) ? 'currentColor' : 'none'} /></button>
          </div>) : <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(query)}`} target="_blank" rel="noreferrer" className="text-xs font-bold text-teal-200 no-underline">YouTube에서 이 키워드 검색하기</a>}
        </div>}
      </div>}
    </section>
  );
