  ...
  return (
    <section ref={searchShellRef} className="music-player-shell border-b border-white/10 bg-[#0b1222] px-3 py-2 text-white shadow-[0_8px_30px_rgba(0,0,0,.18)] sm:px-5">
      <div className="mx-auto flex max-w-[1440px] items-center gap-3">
        ...
        <div className="flex items-center gap-1"><button type="button" onClick={() => selectRelativeTrack(-1)} aria-label="이전 곡" className="music-player-skip inline-flex items-center justify-center p-2 text-slate-400"><SkipBack size={15} /></button><button type="button" onClick={togglePlaying} aria-label={playing ? '일시정지' : '재생'} className="music-player-toggle inline-flex items-center justify-center p-2">{playing ? <Pause size={15} /> : <Play size={15} />}</button><button type="button" onClick={() => selectRelativeTrack(1)} aria-label="다음 곡" className="music-player-skip inline-flex items-center justify-center p-2 text-slate-400"><SkipForward size={15} /></button></div>
        <div className="music-player-search relative flex min-w-[110px] max-w-[360px] flex-1 items-center gap-2 px-2 py-2 sm:min-w-[180px] sm:px-3"><Search size={15} className="shrink-0 text-slate-400" /><input value={query} onFocus={() => setSearchFocused(true)} onBlur={() => window.setTimeout(() => setSearchFocused(false), 160)} onChange={(event) => setQuery(event.target.value)} placeholder="음악 검색 · 핫키워드" className="music-player-search-input w-full text-sm text-white outline-none placeholder:text-slate-500" /></div>
        ...
        <button type="button" onClick={() => toggleFavorite(track)} aria-label="즐겨찾기" className={`music-player-favorite inline-flex items-center justify-center p-2 ${favoriteTracks.some((favorite) => favorite.id === track.id) ? 'text-rose-300' : 'text-slate-400'}`}><Heart size={15} fill={favoriteTracks.some((favorite) => favorite.id === track.id) ? 'currentColor' : 'none'} /></button>
         <Link href="/music" className="music-video-link hidden px-3 py-2 sm:block">MUSIC VIDEO</Link>
      ...
      {searchFocused && <div className="music-player-results mx-auto mt-2 max-w-[1440px] p-2"><div className="mb-2 flex items-center justify-between gap-2"><span className="music-results-label">즐겨찾기 플레이리스트</span><button type="button" disabled={!favoriteTracks.length} onMouseDown={(event) => event.preventDefault()} onClick={() => setFavoritesLoop((value) => !value)} className="music-search-toggle px-2 py-1 text-[10px] disabled:opacity-40">{favoritesLoop ? '즐겨찾기 반복 중' : '즐겨찾기만 반복'}</button></div>{favoriteTracks.length ? <div className="mb-3 grid gap-1.5 sm:grid-cols-2">{favoriteTracks.map((item) => <button type="button" key={`top-favorite-${item.id}`} onMouseDown={(event) => event.preventDefault()} onClick={() => void selectTrack(item)} className="music-search-favorite flex min-w-0 items-center gap-2 p-2 text-left">{item.thumbnail && <img src={item.thumbnail} alt="" className="h-8 w-12 object-cover" />}<span className="min-w-0 truncate text-[11px] font-bold text-slate-300">{item.title}</span></button>)}</div> : <p className="mb-3 text-[10px] text-slate-500">하트 버튼으로 좋아하는 영상을 담아보세요.</p>}<div className="flex flex-wrap gap-1.5">{MUSIC_HOT_KEYWORDS.map((keyword) => <button type="button" key={keyword} onMouseDown={(event) => event.preventDefault()} onClick={() => setQuery(keyword)} className="music-search-keyword px-2.5 py-1.5 text-[11px]">#{keyword}</button>)}</div>{query && <div className="mt-2 grid gap-2 sm:grid-cols-2">{results.length ? results.map((item) => <div key={item.id} className={`music-search-result flex items-center gap-2 px-2 py-2 ${item.id === track.id ? 'is-active' : ''}`}><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => void selectTrack(item)} className="flex min-w-0 flex-1 items-center gap-2 text-left">{item.thumbnail && <img src={item.thumbnail} alt="" className="h-10 w-16 object-cover" />}<span className="min-w-0"><span className="block truncate text-xs font-normal">{item.title}</span><span className="block truncate text-[11px] text-slate-400">{item.artist}</span><span className="block truncate text-[10px] text-slate-500">{item.views || 'YouTube 검색 결과'}{item.published ? ` · ${item.published}` : ''}</span></span></button><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => toggleFavorite(item)} aria-label="즐겨찾기" className={`music-search-heart inline-flex items-center justify-center p-1 ${favoriteTracks.some((favorite) => favorite.id === item.id) ? 'text-rose-300' : 'text-slate-500'}`}><Heart size={14} fill={favoriteTracks.some((favorite) => favorite.id === item.id) ? 'currentColor' : 'none'} /></button></div>) : <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(query)}`} target="_blank" rel="noreferrer" className="text-xs font-normal text-teal-200 underline">YouTube에서 이 키워드 검색하기</a>}</div>}</div>}
    </section>
  );
}
