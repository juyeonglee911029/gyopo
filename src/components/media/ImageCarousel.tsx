...
export default function ImageCarousel({ images = [], alt, className = '', emptyLabel = '등록된 사진이 없습니다.' }: ImageCarouselProps) {
  ...
  const current = uniqueImages[index] || uniqueImages[0];
  ...
  const move = (step: number) => setIndex((value) => (value + step + uniqueImages.length) % uniqueImages.length);

  return (
    <div className={`relative overflow-hidden bg-black/30 ${className}`}>
      <img src={current} alt={alt} className="h-full min-h-48 w-full object-cover" />
      {uniqueImages.length > 1 && <>
        ...
        <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5 rounded-full bg-black/55 px-2.5 py-1.5">
          {uniqueImages.map((image, dotIndex) => <button key={image} type="button" aria-label={`${dotIndex + 1}번째 사진`} aria-pressed={dotIndex === index} onClick={() => setIndex(dotIndex)} className={`h-1.5 w-1.5 rounded-full ${dotIndex === index ? 'bg-cyan-300' : 'bg-white/50'}`} />)}
        </div>
      </>}
    </div>
  );
