...
export async function recordLedgerTransaction(
  entry: Omit<LedgerTransaction, 'createdAt'> & { id: string; createdAt?: string },
  token?: string,
): Promise<void> {
...
export async function approveDepositRequest(requestId: string, userId: string, reviewedBy: string, token?: string): Promise<void> {
  ...
  const ledgerFields = encodeFields({
    userId, type: 'DEPOSIT', amount, status: 'COMPLETED', direction: 'CREDIT',
    details: 'USDT TRC20 입금 승인', requestId, network: request.network || USDT_NETWORK,
    walletAddress: request.sourceWalletAddress || '', txHash: request.txHash || '', createdAt: new Date(), immutable: true,
  });
