...
import { listOnlineUsers, OnlineUser, purchasePremiumSubscription } from '@/lib/firebase';
...
export default function UsersPage() {
  ...
  const activeSubscription = user.isSubscribed && (!user.premiumExpiresAt || new Date(user.premiumExpiresAt).getTime() > Date.now());
  const handleSubscribe = async () => {
    setIsPurchasing(true);
    try {
      const refreshed = await purchasePremiumSubscription(user.id);
      setUser(refreshed);
      window.alert(`프리미엄 구독이 완료되었습니다.${refreshed.premiumExpiresAt ? ` 만료일: ${new Date(refreshed.premiumExpiresAt).toLocaleDateString('ko-KR')}` : ''}`);
    } catch (error) {
      window.alert(error instanceof Error ? error.message : '프리미엄 결제에 실패했습니다.');
    } finally {
