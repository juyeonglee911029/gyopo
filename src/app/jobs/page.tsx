...
type Job = { id: string; title: string; company: string; location: string; salary: string; tag: string; country: string; authorId: string; createdAt: string; sourceContentId?: string };
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <main className="flex-1 space-y-4">
          {filteredJobs.length === 0 && <div className="text-center py-20 bg-gray-50 rounded-xl border border-gray-100"><span className="text-4xl block mb-4">📭</span><p className="text-gray-500">해당 국가의 구인/구직 공고가 없습니다.</p></div>}
           {filteredJobs.map((job) => <Link href={job.sourceContentId ? `/content/${job.sourceContentId}` : `/jobs?job=${job.id}`} key={job.id} className="group block"><div className="flex flex-col justify-between gap-4 rounded-xl border border-gray-100 bg-white p-6 transition-all hover:border-blue-300 hover:shadow-md sm:flex-row sm:items-center"><div className="space-y-1"><div className="mb-2 flex items-center gap-2"><span className="rounded-md bg-gray-100 px-2.5 py-1 text-xs font-bold text-gray-700">[{job.country}]</span><span className="rounded-md bg-blue-100 px-2.5 py-1 text-xs font-bold text-blue-700">{job.tag}</span><span className="text-sm font-medium text-gray-500">{job.company}</span></div><h3 className="text-xl font-bold text-gray-900 transition-colors group-hover:text-blue-600">{job.title}</h3><div className="mt-2 flex items-center gap-4 text-sm text-gray-500"><span>📍 {job.location}</span><span>💰 {job.salary}</span></div></div>{job.sourceContentId && <span className="text-xs font-black text-teal-600">상세 읽기 →</span>}</div></Link>)}
        </main>
      </div>
