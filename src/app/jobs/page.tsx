...
export default function JobsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070b17] px-4 py-8 text-white">
       <div className="mx-auto max-w-5xl"><div className="mb-8 flex items-center justify-between gap-4"><div><h1 className="text-3xl font-black text-white">구인/구직</h1><p className="mt-2 inline-block rounded-full bg-blue-400/10 px-3 py-1 text-sm font-bold text-blue-200">{selectedCountry === 'Global' ? '전체 국가 결과' : `${selectedCountry} 맞춤 검색 결과`}</p></div><button onClick={() => setIsWriting(true)} className="rounded-lg bg-blue-600 px-5 py-2.5 font-bold text-white shadow-md transition hover:bg-blue-500">구인 글쓰기</button></div>
      <div className="mb-8"><BannerAd type="horizontal" /></div>
       <div className="flex flex-col gap-8 md:flex-row">
        <aside className="w-full shrink-0 md:w-64"><div className="rounded-xl border border-white/10 bg-[#10182b] p-5"><h3 className="mb-4 border-b border-white/10 pb-2 text-lg font-bold text-white">안내</h3><p className="text-sm leading-6 text-slate-300">실제 구인 정보만 등록해주세요. 국가 선택 없이 전체 공고를 국가 태그로 구분해 보여드립니다.</p></div><div className="mt-6"><BannerAd type="vertical" /></div></aside>
