...
export default function AppsPage() {
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 text-slate-100 sm:px-6 lg:px-8"><header className="max-w-3xl border-b border-white/10 pb-8"><p className="text-xs font-black uppercase tracking-[.22em] text-violet-300">Tools for connection / 앱</p><h1 className="mt-3 text-4xl font-black tracking-tight text-white sm:text-5xl">필요한 기능을 바로 실행하세요</h1><p className="mt-4 text-sm leading-7 text-slate-300">생활 정보를 찾고, 사람을 만나고, 게임과 음악을 즐기는 GYOPO의 연결 도구입니다.</p></header><main className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{apps.map(({ href, label, english, detail, icon: Icon, tone }) => <Link key={href} href={href} className="group rounded-3xl border border-white/10 bg-white/[.045] p-6 transition hover:-translate-y-0.5 hover:border-violet-300/30 hover:bg-white/[.075]"><div className="flex items-start justify-between"><span className={`flex h-11 w-11 items-center justify-center rounded-2xl ${tone}`}><Icon size={20} /></span><ArrowUpRight size={18} className="text-slate-600 transition group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-teal-300" /></div><h2 className="mt-6 text-xl font-black text-white">{label}<span className="ml-2 text-xs font-semibold text-slate-500">{english}</span></h2><p className="mt-2 text-sm leading-6 text-slate-400">{detail}</p></Link>)}</main></div>
  );
}
