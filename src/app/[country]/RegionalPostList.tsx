      ...
      {posts.map((post) => (
        <li key={`${post.collection}:${post.id}`} className="p-5 sm:p-6">
          ...
          <h3 className="break-words text-lg font-bold text-white">
            <Link href={basePath ? `${basePath}/${encodeURIComponent(post.id)}` : regionalPostHref(post.country, post.category, post.id)} className="hover:text-teal-200">{post.title}</Link>
          </h3>
