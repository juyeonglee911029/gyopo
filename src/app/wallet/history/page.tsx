      ...
      <section className="wallet-history-shell overflow-hidden rounded-3xl">
        ...
        {loading && !rows.length ? <div className="flex items-center justify-center gap-2 p-12 text-sm text-slate-400"><RefreshCw size={16} className="animate-spin" /> 원장을 불러오는 중...</div> : !visibleRows.length ? <div className="p-14 text-center text-sm text-slate-500">선택한 분류의 거래내역이 없습니다.</div> : <>
          <div className="hidden border-b border-white/10 px-5 py-3 text-[10px] font-black uppercase tracking-[.16em] text-slate-500 lg:grid lg:grid-cols-[minmax(170px,1.2fr)_minmax(170px,1fr)_minmax(170px,1fr)_auto_auto] lg:items-center lg:gap-4"><span>거래</span><span>이동 경로</span><span>일시</span><span>금액</span><span>상태</span></div>
           <div>{visibleRows.map((row) => <article key={row.id} className="wallet-history-row border-b border-white/10 last:border-b-0"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`rounded-md px-2 py-1 text-[10px] font-black ${row.direction === 'IN' ? 'bg-emerald-300/15 text-emerald-700' : row.direction === 'OUT' ? 'bg-rose-300/15 text-rose-700' : 'bg-slate-200 text-slate-600'}`}>{kindLabel(row)}</span><span className="truncate text-sm font-bold text-slate-900">{row.memo || row.type}</span></div><p className="mt-1 truncate text-[10px] text-slate-500">{row.network || '서비스 원장'}</p></div><div className="truncate text-xs text-slate-500">{routeLabel(row)}</div><time className="text-xs text-slate-500">{formatDate(row.createdAt)}</time><strong className={`whitespace-nowrap text-right text-sm ${row.direction === 'IN' ? 'text-emerald-700' : row.direction === 'OUT' ? 'text-rose-700' : 'text-slate-700'}`}>{row.direction === 'IN' ? '+' : row.direction === 'OUT' ? '-' : ''}{formatUsdt(Number(row.amount || 0))} USDT</strong><span className={`text-right text-[10px] font-black ${row.status === 'COMPLETED' ? 'text-emerald-700' : row.status === 'PENDING' || row.status === 'SUBMITTED' ? 'text-amber-700' : 'text-slate-500'}`}>{row.status}</span></article>)}</div>
           <div className="wallet-history-mobile">{visibleRows.map((row) => <article key={row.id} className="border-b border-white/10 p-4 last:border-b-0"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className={`inline-flex rounded-md px-2 py-1 text-[10px] font-black ${row.direction === 'IN' ? 'bg-emerald-300/15 text-emerald-700' : row.direction === 'OUT' ? 'bg-rose-300/15 text-rose-700' : 'bg-slate-200 text-slate-600'}`}>{kindLabel(row)}</span><h3 className="mt-2 truncate text-sm font-bold text-slate-900">{row.memo || row.type}</h3></div><strong className={`whitespace-nowrap text-sm ${row.direction === 'IN' ? 'text-emerald-700' : row.direction === 'OUT' ? 'text-rose-700' : 'text-slate-700'}`}>{row.direction === 'IN' ? '+' : row.direction === 'OUT' ? '-' : ''}{formatUsdt(Number(row.amount || 0))}</strong></div><div className="mt-3 grid gap-1 text-xs text-slate-500"><span>이동: {routeLabel(row)}</span><span>일시: {formatDate(row.createdAt)}</span><span>상태: <b className="text-slate-700">{row.status}</b> · {row.network || '서비스 원장'}</span></div>{row.txHash && <a href={`https://tronscan.org/#/transaction/${row.txHash}`} target="_blank" rel="noreferrer" className="mt-2 block truncate font-mono text-[10px] text-cyan-700 hover:underline">TXID: {row.txHash}</a>}</article>)}</div>
        </>}
      </section>
    </div>
  );
  ...
  return (
    <div className="wallet-history-page mx-auto w-full max-w-6xl px-4 py-8 text-slate-100 sm:px-6">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <Link href="/wallet" className="mb-3 inline-flex text-xs font-bold text-cyan-300 hover:text-cyan-100">← 내 지갑</Link>
          <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-cyan-300/15 text-cyan-200"><History size={21} /></span><div><p className="text-[10px] font-black uppercase tracking-[.24em] text-cyan-300">Wallet ledger</p><h1 className="mt-1 text-3xl font-black tracking-tight sm:text-4xl">전문 거래내역</h1></div></div>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">입금·출금·내부 송금·TRON 체인 이동을 한 화면에서 확인합니다. 모든 항목은 서비스 원장에 기록된 시간과 상태를 기준으로 표시됩니다.</p>
        </div>
        <button type="button" onClick={exportCsv} disabled={!rows.length} className="inline-flex items-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-3 py-2 text-xs font-black text-cyan-100 disabled:opacity-40"><Download size={14} /> CSV 내려받기</button>
      </header>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <div className="wallet-history-shell rounded-2xl p-4"><div className="flex items-center gap-2 text-xs font-bold text-emerald-300"><ArrowDownLeft size={14} /> 들어온 금액</div><strong className="mt-2 block text-2xl font-black">{incoming.toFixed(2)} <small className="text-xs text-slate-500">USDT</small></strong></div>
        <div className="wallet-history-shell rounded-2xl p-4"><div className="flex items-center gap-2 text-xs font-bold text-rose-300"><ArrowUpRight size={14} /> 나간 금액</div><strong className="mt-2 block text-2xl font-black">{outgoing.toFixed(2)} <small className="text-xs text-slate-500">USDT</small></strong></div>
        <div className="wallet-history-shell rounded-2xl p-4"><div className="flex items-center gap-2 text-xs font-bold text-amber-300"><WalletCards size={14} /> 처리 대기</div><strong className="mt-2 block text-2xl font-black">{pending} <small className="text-xs text-slate-500">건</small></strong></div>
      </div>
      ...
      <section className="wallet-history-shell overflow-hidden rounded-3xl">
        ...
        {loading && !rows.length ? <div className="flex items-center justify-center gap-2 p-12 text-sm text-slate-400"><RefreshCw size={16} className="animate-spin" /> 원장을 불러오는 중...</div> : !visibleRows.length ? <div className="p-14 text-center text-sm text-slate-500">선택한 분류의 거래내역이 없습니다.</div> : <>
          <div className="hidden border-b border-white/10 px-5 py-3 text-[10px] font-black uppercase tracking-[.16em] text-slate-500 lg:grid lg:grid-cols-[minmax(170px,1.2fr)_minmax(170px,1fr)_minmax(170px,1fr)_auto_auto] lg:items-center lg:gap-4"><span>거래</span><span>이동 경로</span><span>일시</span><span>금액</span><span>상태</span></div>
          <div>{visibleRows.map((row) => <article key={row.id} className="wallet-history-row border-b border-white/10 last:border-b-0"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`rounded-md px-2 py-1 text-[10px] font-black ${row.direction === 'IN' ? 'bg-emerald-300/15 text-emerald-200' : row.direction === 'OUT' ? 'bg-rose-300/15 text-rose-200' : 'bg-white/10 text-slate-300'}`}>{kindLabel(row)}</span><span className="truncate text-sm font-bold text-white">{row.memo || row.type}</span></div><p className="mt-1 truncate text-[10px] text-slate-500">{row.network || '서비스 원장'}</p></div><div className="truncate text-xs text-slate-400">{routeLabel(row)}</div><time className="text-xs text-slate-400">{formatDate(row.createdAt)}</time><strong className={`whitespace-nowrap text-right text-sm ${row.direction === 'IN' ? 'text-emerald-300' : row.direction === 'OUT' ? 'text-rose-300' : 'text-slate-200'}`}>{row.direction === 'IN' ? '+' : row.direction === 'OUT' ? '-' : ''}{Number(row.amount || 0).toFixed(2)} USDT</strong><span className={`text-right text-[10px] font-black ${row.status === 'COMPLETED' ? 'text-emerald-300' : row.status === 'PENDING' || row.status === 'SUBMITTED' ? 'text-amber-300' : 'text-slate-500'}`}>{row.status}</span></article>)}</div>
          <div className="wallet-history-mobile">{visibleRows.map((row) => <article key={row.id} className="border-b border-white/10 p-4 last:border-b-0"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className={`inline-flex rounded-md px-2 py-1 text-[10px] font-black ${row.direction === 'IN' ? 'bg-emerald-300/15 text-emerald-200' : row.direction === 'OUT' ? 'bg-rose-300/15 text-rose-200' : 'bg-white/10 text-slate-300'}`}>{kindLabel(row)}</span><h3 className="mt-2 truncate text-sm font-bold">{row.memo || row.type}</h3></div><strong className={`whitespace-nowrap text-sm ${row.direction === 'IN' ? 'text-emerald-300' : row.direction === 'OUT' ? 'text-rose-300' : 'text-slate-200'}`}>{row.direction === 'IN' ? '+' : row.direction === 'OUT' ? '-' : ''}{Number(row.amount || 0).toFixed(2)}</strong></div><div className="mt-3 grid gap-1 text-xs text-slate-500"><span>이동: {routeLabel(row)}</span><span>일시: {formatDate(row.createdAt)}</span><span>상태: <b className="text-slate-300">{row.status}</b> · {row.network || '서비스 원장'}</span></div>{row.txHash && <a href={`https://tronscan.org/#/transaction/${row.txHash}`} target="_blank" rel="noreferrer" className="mt-2 block truncate font-mono text-[10px] text-cyan-300 hover:underline">TXID: {row.txHash}</a>}</article>)}</div>
        </>}
      </section>
    </div>
  );
