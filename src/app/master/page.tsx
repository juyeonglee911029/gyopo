...
export default function MasterPage() {
  ...
  const syncSource = async (source: ContentSource) => {
    ...
    try {
      ...
      try {
        await mergeDocument('contentSnapshots', source.id, snapshot, token);
      } catch {
        // Production may still have the old Firestore rules. Posts is the public fallback publishing channel.
        await mergeDocument('posts', `source-snapshot-${source.id}`, {
          type: 'news',
          sourceSnapshot: true,
          sourceId: source.id,
      ...
      for (const section of data.sections || []) {
        for (const item of section.items || []) {
          ...
          if (section.category === 'jobs') {
            await mergeDocument('jobs', id, { title: item.title, company: source.name, location: source.region, salary: '원문 확인', tag: '출처 자동수집', country: source.region, authorId, createdAt, sourceUrl: item.url, sourceName: source.name }, token);
          } else if (section.category === 'directory') {
            await mergeDocument('directories', id, { name: item.title, category: source.name, desc: item.description || '공식 출처에서 확인된 정보입니다.', tel: '원문 확인', address: source.region, rating: 0, reviews: 0, country: source.region, authorId, createdAt, sourceUrl: item.url, sourceName: source.name }, token);
          } else if (section.category === 'market') {
            await mergeDocument('marketItems', id, { title: item.title, price: '원문 확인', location: source.region, country: source.region, authorId, createdAt, sourceUrl: item.url, sourceName: source.name }, token);
          } else if (section.category === 'community' || section.category === 'news' || section.category === 'events') {
            await mergeDocument('posts', id, { type: 'news', title: item.title, body: item.description || `${source.name} 원문에서 확인하세요.`, authorId, author: source.name, country: source.region, createdAt, sourceUrl: item.url, sourceName: source.name }, token);
          }
        }
      }
      setSourceStatus((current) => ({ ...current, [source.id]: '게시 완료' }));
      setMessage(`${source.name}: 출처 확인 후 사이트 카테고리에 게시했습니다.`);
    } catch (error) {
