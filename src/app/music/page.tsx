    ...
    <div className="music-page min-h-screen bg-[#070b17] px-4 py-8 text-white md:px-8 md:py-12">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-5">
           <div><div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[0.28em] text-teal-300"><Music2 size={16} /> MUSIC VIDEO</div><h1 className="text-4xl font-black tracking-tight md:text-6xl">MUSIC VIDEO</h1><p className="mt-3 text-sm text-slate-400">등록곡과 YouTube 뮤직비디오를 검색하고 즐겨찾기 목록으로 계속 재생합니다.</p></div>
          <div className="flex flex-wrap gap-2">{MUSIC_HOT_KEYWORDS.map((keyword) => <button type="button" key={keyword} onClick={() => setQuery(keyword)} className="rounded-none border-0 bg-white/10 px-3 py-2 text-xs font-bold text-slate-300 hover:bg-white/20 hover:text-white">#{keyword}</button>)}</div>
        </div>

        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_330px]">
          <section className="overflow-hidden rounded-none border-0 bg-[#10182b] shadow-2xl">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-5 md:p-7"><div><div className="text-xs font-black uppercase tracking-[0.2em] text-teal-300">Now playing</div><h2 className="mt-2 text-3xl font-black">{track.title}</h2><p className="mt-1 text-sm font-bold text-slate-400">{track.artist}</p><div className="mt-3 flex flex-wrap gap-2 text-[11px] font-bold text-slate-500"><span className="border-0 bg-white/[.08] px-2.5 py-1.5">조회수 · {track.views || '집계 중'}</span><span className="border-0 bg-white/[.08] px-2.5 py-1.5">발매일 · {track.published || '정보 확인 중'}</span></div></div><div className="flex items-center gap-3"><button type="button" onClick={togglePlaying} className="flex items-center gap-2 rounded-none border-0 bg-white/15 px-3 py-2 text-xs font-black text-white">{playing ? <Pause size={14} /> : <Play size={14} />}{playing ? '일시정지' : '재생'}</button><label className="flex items-center gap-2 text-xs text-slate-400">볼륨<input type="range" min="0" max="100" value={volume} onChange={(event) => changeVolume(Number(event.target.value))} className="accent-teal-300" /></label></div></div>
          </section>

          <aside className="rounded-none border-0 bg-[#10182b] p-5">
            <div className="flex items-center gap-2 rounded-none border-0 bg-white/10 px-3 py-2"><Search size={16} className="text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="YouTube 곡·가수 검색" className="w-full rounded-none border-0 bg-transparent text-sm outline-none placeholder:text-slate-500" /></div>
            <div className="mt-5 flex items-center justify-between gap-2"><h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-300">즐겨찾기 플레이리스트</h3><button type="button" disabled={!favoriteTracks.length} onClick={() => setFavoritesLoop((value) => !value)} className="rounded-none border-0 bg-white/10 px-2 py-1 text-[10px] font-black text-white disabled:opacity-40">{favoritesLoop ? '즐겨찾기 반복 중' : '즐겨찾기만 반복'}</button></div>
            <div className="mt-2 space-y-2">{favoriteTracks.length ? favoriteTracks.map((item) => <div key={`favorite-${item.id}`} className="flex items-center gap-2 bg-white/[.06] p-2"><button type="button" onClick={() => void selectTrack(item)} className="flex min-w-0 flex-1 items-center gap-2 text-left">{item.thumbnail && <img src={item.thumbnail} alt="" className="h-9 w-14 shrink-0 object-cover" />}<span className="min-w-0"><b className="block truncate text-xs">{item.title}</b><span className="block truncate text-[10px] text-slate-400">{item.artist}</span></span></button><button type="button" onClick={() => toggleFavorite(item)} aria-label="즐겨찾기 해제" className="inline-flex items-center justify-center border-0 bg-transparent p-1 text-rose-300"><Heart size={14} fill="currentColor" /></button></div>) : <p className="bg-white/[.04] p-2 text-[11px] text-slate-500">하트 버튼으로 좋아하는 뮤직비디오를 담아보세요.</p>}</div>
            <div className="mt-6 flex items-center justify-between gap-2"><h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-300">YouTube 검색 결과</h3><button type="button" onClick={() => setFavoritesOnly((value) => !value)} className="rounded-none border-0 bg-white/10 px-2 py-1 text-[10px] font-black text-white">{favoritesOnly ? '전체 보기' : '즐겨찾기만 보기'}</button></div>
            <div className="mt-3 space-y-2">{results.map((item) => <div key={item.id} className={`flex items-center gap-2 rounded-none border-0 p-2 ${track.id === item.id ? 'bg-white/15' : 'bg-white/5'}`}><button type="button" onClick={() => void selectTrack(item)} className="flex min-w-0 flex-1 items-center gap-3 text-left">{item.thumbnail ? <img src={item.thumbnail} alt="" className="h-12 w-20 shrink-0 object-cover" /> : <span className="flex h-9 w-9 shrink-0 items-center justify-center bg-slate-900 text-teal-300"><Play size={14} fill="currentColor" /></span>}<span className="min-w-0"><b className="block truncate text-sm">{item.title}</b><span className="block truncate text-xs text-slate-400">{item.artist}</span><span className="block truncate text-[10px] text-slate-500">{item.views || '등록곡'}{item.published ? ` · ${item.published}` : ''}</span></span></button><button type="button" onClick={() => toggleFavorite(item)} aria-label={favoriteTracks.some((favorite) => favorite.id === item.id) ? '즐겨찾기 해제' : '즐겨찾기 추가'} className={`inline-flex items-center justify-center border-0 bg-transparent p-1 ${favoriteTracks.some((favorite) => favorite.id === item.id) ? 'text-rose-300' : 'text-slate-500'}`}><Heart size={15} fill={favoriteTracks.some((favorite) => favorite.id === item.id) ? 'currentColor' : 'none'} /></button></div>)}{!results.length && <p className="bg-white/[.04] p-3 text-sm text-slate-500">즐겨찾기 목록이 비어 있습니다.</p>}</div>
          </aside>
        </div>
      </div>
    </div>
