...
import { createDocument, getSessionToken, listDocuments, listEscrowOrdersForMember, mergeDocument, reserveEscrowPurchase, type EscrowOrder, type EscrowStatus } from '@/lib/firebase';
...
export default function MarketPage() {
  ...
  const loadProducts = async () => {
    try {
      const [data, source] = await Promise.all([
        listDocuments<Omit<Product, 'id'>>('marketItems', getSessionToken()),
        selectedCountry === 'Brazil' || selectedCountry === 'Global' ? fetchSourceCategory('hanintoday-brazil', 'market') : Promise.resolve(null),
      ]);
      const sourceProducts = source?.items.map((item) => ({ id: sourceItemId('hanintoday-brazil', 'market', item.url), title: item.title, price: '원문 확인', location: source.region, image: item.image, country: source.region, authorId: 'source', createdAt: item.publishedAt || source.fetchedAt, sourceUrl: item.url, sourceName: source.sourceName, sourceContentId: sourceItemId('hanintoday-brazil', 'market', item.url) })) || [];
      setProducts([...sourceProducts, ...data].filter((item) => item.authorId).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
     } catch { setProducts([]); }
  };
  ...
  useEffect(() => {
    ...
    void listEscrowOrdersForMember(user.id, getSessionToken()).then(setOrders).catch(() => setOrders([]));
  }, [user?.id]);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
       ...
       {filteredProducts.length === 0 && <div className="text-center py-20 bg-gray-50 rounded-xl border border-gray-100"><span className="text-4xl block mb-4">🛒</span><p className="text-gray-500">등록된 매물이 없습니다.</p></div>}
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 md:gap-6">{filteredProducts.map((item) => <div key={item.id} className="group relative overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm transition-all hover:shadow-lg"><div className="absolute left-2 top-2 z-10"><span className="rounded bg-black/60 px-2 py-1 text-[10px] font-bold text-white">{item.country}</span></div><div className="flex aspect-square items-center justify-center overflow-hidden bg-gray-100">{item.image ? <img src={item.image} alt={item.title} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" /> : <span className="text-4xl text-gray-400">📦</span>}</div><div className="space-y-2 p-4"><h3 className="truncate font-medium text-gray-900 group-hover:text-orange-600">{item.title}</h3><div className="inline-block rounded bg-green-50 px-2 py-1 text-lg font-black text-green-600">{item.price}</div><div className="flex justify-between border-t border-gray-50 pt-2 text-xs text-gray-500"><span>{item.location}</span><span>{new Date(item.createdAt).toLocaleDateString('ko-KR')}</span></div>{item.sourceName && <div className="text-[10px] text-blue-500">출처: {item.sourceName}</div>}{item.sourceContentId ? <Link href={`/content/${item.sourceContentId}?source=hanintoday-brazil&category=market&url=${encodeURIComponent(item.sourceUrl || '')}`} className="mt-2 block w-full rounded-xl border border-orange-200 py-2 text-center text-xs font-black text-orange-600">상세 읽기</Link> : <button onClick={() => void handleBuy(item)} className="mt-2 w-full rounded-xl bg-orange-500 py-2 text-xs font-black text-white hover:bg-orange-600">구매 · 에스크로 보관</button>}</div></div>)}</div>
        {orders.length > 0 && <section className="mt-10 rounded-2xl border border-gray-200 bg-white p-5"><h2 className="mb-4 text-xl font-black">내 에스크로 진행 내역</h2><div className="space-y-3">{orders.map((order) => <div key={order.id} className="rounded-xl border border-gray-100 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-bold">{order.amount} USDT · {order.status}</span><div className="flex gap-2">{order.sellerId === user?.id && order.status === 'PAYMENT_HELD' && <button onClick={() => void updateOrder(order, 'SHIPPING', '판매자가 배송을 시작했습니다.')} className="rounded-lg bg-cyan-500 px-3 py-2 text-xs font-black text-white">배송 시작</button>}{order.sellerId === user?.id && order.status === 'SHIPPING' && <button onClick={() => void updateOrder(order, 'IN_TRANSIT', '상품이 배송 중입니다.')} className="rounded-lg bg-blue-500 px-3 py-2 text-xs font-black text-white">배송 중으로 변경</button>}{order.buyerId === user?.id && order.status === 'IN_TRANSIT' && <button onClick={() => void updateOrder(order, 'DELIVERED', '구매자가 수령을 확인했습니다.')} className="rounded-lg bg-emerald-500 px-3 py-2 text-xs font-black text-white">수령 확인</button>}</div></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs text-gray-500 sm:grid-cols-4">{(['PAYMENT_HELD', 'SHIPPING', 'IN_TRANSIT', 'DELIVERED'] as EscrowStatus[]).map((step) => <div key={step} className={`rounded-lg px-2 py-2 text-center ${order.timeline?.some((item) => item.status === step) || order.status === step ? 'bg-emerald-100 font-bold text-emerald-700' : 'bg-gray-100'}`}>{step}</div>)}</div></div>)}</div></section>}
