  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      {filteredProducts.length === 0 && <div className="text-center py-20 bg-gray-50 rounded-xl border border-gray-100"><span className="text-4xl block mb-4">🛒</span><p className="text-gray-500">등록된 매물이 없습니다.</p></div>}
       <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">{filteredProducts.map((item) => <Link href={item.sourceUrl || `/market?item=${item.id}`} key={item.id} className="group" target={item.sourceUrl ? '_blank' : undefined}><div className="bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-all relative"><div className="absolute top-2 left-2 z-10"><span className="bg-black/60 text-white text-[10px] font-bold px-2 py-1 rounded">{item.country}</span></div><div className="aspect-square overflow-hidden bg-gray-100 flex items-center justify-center">{item.image ? <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" /> : <span className="text-gray-400 text-4xl">📦</span>}</div><div className="p-4 space-y-2"><h3 className="text-gray-900 font-medium truncate group-hover:text-orange-600">{item.title}</h3><div className="font-black text-lg text-green-600 bg-green-50 px-2 py-1 rounded inline-block">{item.price}</div><div className="flex justify-between text-xs text-gray-500 pt-2 border-t border-gray-50"><span>{item.location}</span><span>{new Date(item.createdAt).toLocaleDateString('ko-KR')}</span></div>{item.sourceName && <div className="text-[10px] text-blue-500">출처: {item.sourceName}</div>}</div></div></Link>)}</div>
