            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 md:gap-6">
              {filteredProducts.map((item) => (
                <div key={item.id} className="group relative overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm transition-all hover:shadow-lg">
                  <Link href={`/content/${encodeURIComponent(item.id)}?collection=marketItems`} className="group block">
                    <div className="absolute left-2 top-2 z-10"><span className="rounded bg-black/60 px-2 py-1 text-[10px] font-bold text-white">{item.country}</span></div>
                    <div className="flex aspect-square items-center justify-center overflow-hidden bg-gray-100">{item.image ? <img src={item.image} alt={item.title} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" /> : <span className="text-4xl text-gray-400">📦</span>}</div>
                    <div className="space-y-2 p-4"><h3 className="truncate font-medium text-gray-900 group-hover:text-orange-600">{item.title}</h3><div className="inline-block rounded bg-green-50 px-2 py-1 text-lg font-black text-green-600">{item.price}</div><div className="flex justify-between border-t border-gray-50 pt-2 text-xs text-gray-500"><span>{item.location}</span><span>{new Date(item.createdAt).toLocaleDateString('ko-KR')}</span></div></div>
                  </Link>
                  <div className="px-4 pb-4"><button onClick={() => void handleBuy(item)} className="mt-2 w-full rounded-xl bg-orange-500 py-2 text-xs font-black text-white hover:bg-orange-600">구매 · 에스크로 보관</button>{user && isNativeProduct(item) && (user.id === item.authorId || isMasterUser(user)) && <button onClick={() => void removeProduct(item)} className="w-full py-1 text-xs font-bold text-red-500">삭제</button>}</div>
                </div>
              ))}
            </div>
