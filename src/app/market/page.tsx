...
type Product = { id: string; title: string; price: string; location: string; image?: string; country: string; authorId: string; createdAt: string; sourceUrl?: string; sourceName?: string };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
       ...
       {filteredProducts.length === 0 && <div className="text-center py-20 bg-gray-50 rounded-xl border border-gray-100"><span className="text-4xl block mb-4">🛒</span><p className="text-gray-500">등록된 매물이 없습니다.</p></div>}
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 md:gap-6">{filteredProducts.map((item) => <div key={item.id} className="group bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-all relative"><div className="absolute top-2 left-2 z-10"><span className="bg-black/60 text-white text-[10px] font-bold px-2 py-1 rounded">{item.country}</span></div><div className="aspect-square overflow-hidden bg-gray-100 flex items-center justify-center">{item.image ? <img src={item.image} alt={item.title} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300" /> : <span className="text-gray-400 text-4xl">📦</span>}</div><div className="p-4 space-y-2"><h3 className="text-gray-900 font-medium truncate group-hover:text-orange-600">{item.title}</h3><div className="font-black text-lg text-green-600 bg-green-50 px-2 py-1 rounded inline-block">{item.price}</div><div className="flex justify-between text-xs text-gray-500 pt-2 border-t border-gray-50"><span>{item.location}</span><span>{new Date(item.createdAt).toLocaleDateString('ko-KR')}</span></div>{item.sourceName && <div className="text-[10px] text-blue-500">출처: {item.sourceName}</div>}{!item.sourceUrl && <button onClick={() => void handleBuy(item)} className="mt-2 w-full rounded-xl bg-orange-500 py-2 text-xs font-black text-white hover:bg-orange-600">구매 · 에스크로 보관</button>}</div></div>)}</div>
