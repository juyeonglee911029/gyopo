...
export default function DirectoryPage() {
  ...
  const loadDirectories = async () => {
    try {
      ...
      const sourceDirectories = source?.items.map((item) => ({ id: sourceItemId('hanintoday-brazil', 'directory', item.url), name: item.title, category: '출처 업소록', desc: item.description || '원문 출처에서 확인된 업소 정보입니다.', body: item.body, tel: '원문 확인', address: source.region, rating: 0, reviews: 0, country: source.region, image: item.image, images: item.images, authorId: 'source', createdAt: item.publishedAt || source.fetchedAt, sourceUrl: item.url, sourceName: source.sourceName, sourceContentId: sourceItemId('hanintoday-brazil', 'directory', item.url) })) || [];
