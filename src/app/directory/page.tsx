...
export default function DirectoryPage() {
  ...
  const loadDirectories = async () => {
    try {
      ...
      setDirectories([...sourceDirectories, ...data].filter((directory) => directory.authorId).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (form.image && !/^https?:\/\//i.test(form.image.trim())) return window.alert('대표 이미지 URL은 http 또는 https 주소만 사용할 수 있습니다.');
    try {
